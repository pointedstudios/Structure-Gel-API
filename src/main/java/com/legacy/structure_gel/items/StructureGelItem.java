...
public class StructureGelItem extends BlockItem
{
	...
	public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn)
	{
		...
		else
		{
			...
			{
				tooltip.add(new TranslationTextComponent("info." + StructureGelMod.MODID + ".place").mergeStyle(TextFormatting.GRAY));
				tooltip.add(new TranslationTextComponent("info." + StructureGelMod.MODID + ".gunpowder").mergeStyle(TextFormatting.GRAY));
				tooltip.add(new TranslationTextComponent(""));

				if (((StructureGelBlock) this.getBlock()).behaviors.isEmpty())
					tooltip.add(new TranslationTextComponent(Behavior.DEFAULT.getTranslation()).mergeStyle(TextFormatting.GRAY));

				for (IBehavior behavior : ((StructureGelBlock) this.getBlock()).behaviors)
					tooltip.add(new TranslationTextComponent(behavior.getTranslation()).mergeStyle(TextFormatting.GRAY));
			}
			else
				tooltip.add(new TranslationTextComponent("info." + StructureGelMod.MODID + ".unknown_behavior").mergeStyle(TextFormatting.RED));
		}
	}
}
